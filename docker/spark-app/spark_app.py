...
def init_logging():
    ...
    try:
        ...
        return logging.getLogger("UnifiedKafkaStreamApp")
    except PermissionError:
        # Fallback for test mode or restricted environments
        logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
        logger = logging.getLogger("UnifiedKafkaStreamApp")
        logger.warning("⚠️ Falling back to console logging. Cannot write to /mnt/spark.")
        return logger
